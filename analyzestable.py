...
class TokenAnalyzer:
    ...
    def get_address_type(self, address: str) -> str:
        """Determine if address is a contract or user account"""
    ...
    def get_top_holders(self, token_address: str, limit: int = None) -> List[str]:
        """Get top token holders using Covalent API with fallback to on-chain scanning"""
        logger.info(f"Getting top holders for {token_address}")
        holders = self._get_holders_from_covalent(token_address, limit)
        if holders:
            logger.info("Successfully retrieved holders from Covalent API")
            return [self.w3_base.to_checksum_address(addr) for addr in holders]
        else:
            logger.info("Falling back to on-chain scanning...")
            holders = self._get_holders_from_chain(token_address, limit or self.num_holders)
            return [self.w3_base.to_checksum_address(addr) for addr in holders]

    def _get_holders_from_covalent(self, token_address: str, limit: int = None) -> List[str]:
            """Get token holders using pagination to reach desired number"""
            target_holders = limit if limit is not None else self.num_holders
            checksummed_token = self.w3_base.to_checksum_address(token_address)
            all_holders = []
            page = 0
            
            logger.info(f"Fetching holders from Covalent API for {checksummed_token}")
            
            while len(all_holders) < target_holders:
                try:
                    ...
                    logger.debug(f"Making Covalent API request - Page {page + 1}")
                    ...
                    if response.status_code == 200:
                        ...
                        if not items:
                            logger.info("No more holders found in Covalent API")
                            break
                        ...
                        all_holders.extend(valid_holders)
                        logger.info(f"Found {len(valid_holders)} holders on page {page + 1}")
                        ...
                        logger.error(f"Covalent API error: {response.status_code}")
                        logger.error(f"Response: {response.text}")
                        break
                        
                except Exception as e:
                    logger.error(f"Error fetching holders from Covalent: {str(e)}")
                    break
            
            logger.info(f"Total holders found from Covalent: {len(all_holders)}")
            return all_holders[:target_holders]


    def _get_holders_from_chain(self, token_address: str, limit: int = 100) -> List[str]:
        """Fallback method: Get holders by scanning transfer events"""
        checksummed_token = self.w3_base.to_checksum_address(token_address)
        transfer_event_signature = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef'
        
        logger.info(f"Starting on-chain holder scanning for {checksummed_token}")
        
        current_block = self.w3_base.eth.block_number
        ...
        for chunk_start in range(from_block, current_block, 2000):
            chunk_end = min(chunk_start + 2000, current_block)
            try:
                logger.debug(f"Scanning blocks {chunk_start} to {chunk_end}")
                logs = self.w3_base.eth.get_logs({
                ...
                for log in logs:
                    from_addr = self.w3_base.to_checksum_address('0x' + log['topics'][1].hex()[-40:])
                    to_addr = self.w3_base.to_checksum_address('0x' + log['topics'][2].hex()[-40:])
                ...
                logger.info(f"Processed {len(logs)} transfer events in blocks {chunk_start} to {chunk_end}")
            except Exception as e:
                logger.error(f"Error scanning chunk {chunk_start}-{chunk_end}: {str(e)}")
                continue
        
        logger.info("Verifying current balances...")
        ...
        for holder in holder_balances:
            try:
                ...
                if balance > 0:
                    verified_balances[holder] = balance
                    logger.debug(f"Verified balance for {holder[:8]}...")
            except Exception as e:
                logger.error(f"Error verifying balance for {holder}: {str(e)}")
                continue
        
        logger.info(f"Found {len(verified_balances)} holders with positive balances")
        
        # Sort and return top holders
        sorted_holders = sorted(verified_balances.items(), key=lambda x: x[1], reverse=True)
        return [holder[0] for holder in sorted_holders[:limit]]
    ...
    def generate_csv_report(self, analysis_results: Dict[str, Any], output_file: str):
        """Enhanced CSV report with holder type indication"""
        rows = []
        ...
        for holder_data in analysis_results["holders_analysis"]:
            if holder_data:
                row = {
                    ...
                    "Token Balance": holder_data["token_balance"],
                    "% of Total Supply": f"{holder_data['balance_percentage']:.4f}%",
    ...
    def get_total_supply(self, token_address: str) -> float:
        ...
        try:
            # First try Covalent API for total supply
            url = f"https://api.covalenthq.com/v1/8453/tokens/{token_address}/"
            headers = {
                "Authorization": f"Bearer {self.covalent_api_key}"
            }
            ...
            if response.status_code == 200:
                ...
                if 'items' in data and data['items']:
                    return float(data['items'][0]['total_supply'])
        except Exception as e:
            print(f"Error getting total supply from Covalent: {str(e)}")
        
        # Fallback to contract call if available
        try:
            abi = self.erc20_abi + [{"constant":True,"inputs":[],"name":"totalSupply","outputs":[{"name":"","type":"uint256"}],"type":"function"}]
            token_contract = self.w3_base.eth.contract(address=token_address, abi=abi)
            total_supply = token_contract.functions.totalSupply().call()
            decimals = token_contract.functions.decimals().call()
            return total_supply / (10 ** decimals)
        except Exception as e:
            print(f"Error getting total supply from contract: {str(e)}")
            return 0
